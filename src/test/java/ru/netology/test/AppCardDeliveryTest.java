...
public class AppCardDeliveryTest {
    ...
    public void shouldRegistered() {
        ...
        element(".notification__content").shouldHave(Condition.visible).
                waitUntil(Condition.text(generateDate(daysAfterToday)), 10000);
        element(".button__text").click();
        element("[data-test-id=date] [placeholder=\"Дата встречи\"]").
                sendKeys(Keys.chord(Keys.SHIFT, Keys.HOME), Keys.BACK_SPACE, generateDate(daysAfterTodayForTest));
        element(".notification__content").shouldHave(Condition.visible).
                waitUntil(Condition.text(generateDate(daysAfterTodayForTest)), 10000);
    }
